...
class GroupCipher {
    ...
    async decrypt(senderKeyMessageBytes) {
        return await this.queueJob(async () => {
            const record = await this.senderKeyStore.loadSenderKey(this.senderKeyName);
            if (!record) {
                console.error('[GroupCipher] No SenderKeyRecord found for:', this.senderKeyName.toString());
            ...
            if (!senderKeyState) {
                console.error('[GroupCipher] No session state found for:', {
                    senderKeyName: this.senderKeyName.toString(),
                    keyId: senderKeyMessage.getKeyId(),
                    availableStates: record.getSenderKeyStates ? record.getSenderKeyStates().length : 'unknown'
                });
                throw new Error(`No session found to decrypt message from ${this.senderKeyName.toString()} with keyId ${senderKeyMessage.getKeyId()}`);
            }
