...
const makeMessagesRecvSocket = (config) => {
    ...
    const sendMessagesAgain = async (key, ids, retryNode) => {
        ...
        if ((0, WABinary_1.isJidGroup)(remoteJid)) {
            const senderKeyName = `${remoteJid}::${participant}`;
            logger.debug({ remoteJid, participant, senderKeyName }, 'clearing sender key for specific participant on retry');
            await authState.keys.set({ 'sender-key': { [senderKeyName]: null } });

            const result = await authState.keys.get('sender-key-memory', [remoteJid]);
            const senderKeyMap = result[remoteJid] || {};
            if (senderKeyMap[participant]) {
                delete senderKeyMap[participant];
                await authState.keys.set({ 'sender-key-memory': { [remoteJid]: senderKeyMap } });
            }
        }
